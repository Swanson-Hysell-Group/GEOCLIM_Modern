...
\noindent
$k_{e}$ is a proportionality constant ((m/yr)$^{1-m}$) and $m$ and $n$ are adjustable exponents that are kept as 0.5 and 1 as in \citet{Maffre2018a}. This formulation is directly inspired from the stream power law \citep{Davy2000a}. This law and these exponent values are supported by various compilations (e.g. \citealp{Lague2013a}) although \citet{Lague2013a} also suggests that there is variability in the proportionality constant that is difficult to capture at a global scale.
...
\begin{figure}[h!]
    ...
    \caption{Chemical weathering and regolith thickness maps associated with the GEOCLIM model run using the parameter combination that produced that highest $r^{2}$ (Fig. \ref{fig:r2_cross_plot}B). Parameter values used for this model run are: $k_{d}=5\times10^{-4}$, $k_{w}=1$, $\sigma=-0.4$, $k_{rp}=1\times10^{-2}$, metamorphic$_{Ca+Mg}=2500$~mol/m$^{3}$, sediment$_{Ca+Mg}=2000$~mol/m$^{3}$.}
